...
import {
  ...
  PhoneCodeSelect,
  COUNTRIES,
} from "@/components/utils/CountryDropdowns";
...
export const SignupForm = () => {
  ...
  const onSubmit = async (data: JoinWaitlistDTO) => {
    try {
      ...
      router.push(
        `/beta/confirmed?email=${encodeURIComponent(result.email)}&position=${result.position}`,
      );
    } catch (err: any) {
    ...
    >
      ...
      <div className="flex flex-col gap-4 md:flex-row">
        <div className="w-1/3 flex flex-col gap-2">
          ...
          {(() => {
            const { onChange: formOnChange, ...restCodeReg } =
        ...
        <div className="w-2/3 flex flex-col gap-2">
          ...
          {(() => {
            const { onChange: formOnChange, ...restPhoneReg } =
              register("phone");
            return (
              <input
                {...restPhoneReg}
                onChange={(e) => {
                  ...
                  const iso = COUNTRIES.find((c) => c.code === currentCode)
      ...
      <div className="flex flex-col gap-2">
        ...
        {(() => {
          const { onChange: formOnChange, ...restCountryReg } =
            register("country");
          return (
            <CountrySelect
              {...restCountryReg}
              onChange={(e) => {
                ...
                const matchingData = COUNTRIES.find(
                  (c) => c.name === selectedCountry,
                );
                if (matchingData) {
                  setValue("countryCode", matchingData.code, {
                    shouldValidate: true,
                  });
                }
              }}
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          ...
          <option value="club">
            Club Player (Local tournaments/online ratings)
          </option>
          <option value="competitive">Competitive (Titled/Elite Level)</option>
        </select>
        {errors.chessLevel && (
          <span className="text-error text-sm">
            {errors.chessLevel.message}
          </span>
        )}
      </div>
