...
export const walletController = {
  ...
  async initiateWithdrawal(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!bankAccount || !bankAccount.accountNumber) {
        res.status(400).json(error('Add a bank account in Settings before withdrawing'));
        return;
      }
  ...
  async saveBankAccount(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { accountName } = await flutterwaveService.resolveAccount(bankCode, accountNumber);
